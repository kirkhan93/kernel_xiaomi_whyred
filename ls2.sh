...
#
# Pull in linux-stable updates to a kernel tree Parse the provided 
# parameters
function parse_parameters() {
    while (( ${#} )); do
        case ${1} in
            ...
            "-s"|"--single-version")
                UPDATE_MODE=0 ;;
            # Pulls the requested tag from Sasha Levin's linux-stable 
            # repo
            "-t"|"--tag")
                shift
...
function generate_versions() {
    ...
    LATEST_VERSION=$(git tag --sort=-taggerdate -l 
v"${CURRENT_MAJOR_VERSION}"* | head -n 1 | sed s/v//)
    ...
    # UPDATE_MODES: 0. Update one version 1. Update to a specified 
    # version 2. Update to the latest version
    case ${UPDATE_MODE} in
        0)
